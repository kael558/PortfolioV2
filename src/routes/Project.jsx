...
const Project = () => {
	...
	const baseUrl = projectData?.folder
		? `${import.meta.env.BASE_URL}projects/${projectData.folder}`
		: null;
	...
	const slug = projectData.title.replace(/\s+/g, '-').toLowerCase();

	return (
		<div className="min-h-screen bg-gray-900 text-gray-100">
			<Helmet>
				<title>{projectData.title} — Rahel Gunaratne</title>
				<meta name="description" content={projectData.description} />
				<meta property="og:title" content={`${projectData.title} — Rahel Gunaratne`} />
				<meta property="og:description" content={projectData.description} />
				<meta property="og:url" content={`https://rahelgunaratne.com/project/${slug}`} />
				<meta property="og:type" content="article" />
				<link rel="canonical" href={`https://rahelgunaratne.com/project/${slug}`} />
			</Helmet>
			{/* Back button */}
			...
			<article className="max-w-3xl mx-auto px-5 sm:px-8 pt-24 md:pt-28 pb-24">
				...
				{baseUrl && (
					<img
						src={`${baseUrl}/cover.png`}
						alt={`${projectData.title} cover`}
						className="w-full rounded-xl mb-10 object-cover max-h-72"
